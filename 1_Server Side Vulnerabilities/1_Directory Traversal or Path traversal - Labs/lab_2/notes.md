...
Concept : Path traversal is a vulnerability when an application uses user-controlled input to construct
          ...
          - Even developed added 'FIlters for ../../../' An attacker can manipulate using absolute path 
            traversal sequences such as '/etc/passwd' to acces files outside the intended directory.
...
Manual PenTest via burpsuite :
    ...
    5. Exploited this path traversal vulnerability simple case sucessfully

Remediation

    - Validate file names against an allowlist.
    - Reject traversal sequences such as ../ and ..\.
    - Use canonical path validation before accessing files.
    - Store user-accessible files outside sensitive system directories.